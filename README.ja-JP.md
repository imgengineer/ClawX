...
├── installer-tauri/         # オフラインセットアップウィザード（Tauri + React）
...
pnpm run build:installer -- --os=win --arch=x64   # オフラインインストーラーをビルド
pnpm run build:installer -- --os=linux --arch=arm64  # Linux arm64 向けオフラインインストーラーをビルド（Linux ホスト/CI 上で実行）
```
### 技術スタック
